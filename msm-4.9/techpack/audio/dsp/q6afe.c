...
#define AFE_PARAM_ID_TFADSP_RX_SET_MODEL		(0x1000B98E)
		...
		union afe_spkr_prot_config *prot_config)
{
	...
	case AFE_PARAM_ID_TFADSP_RX_SET_MODEL:
...
{
	int32_t result = 0, port_id = AFE_PORT_ID_QUATERNARY_MI2S_RX;
...
/*Attention: port_id = AFE_PORT_ID_TERTIARY_MI2S_RX or AFE_PORT_ID_QUATERNARY_MI2S_RX*/
int send_tfa_set_model(void *buf, int cmd_size)
{
	union afe_spkr_prot_config afe_spk_config;
	...
	if (afe_spk_prot_prepare(port_id, 0,
			AFE_PARAM_ID_TFADSP_RX_SET_MODEL,
			&afe_spk_config)) {
			pr_err("%s: AFE_PARAM_ID_TFADSP_RX_SET_MODEL failed\n",
				   __func__);
	}
...
int send_tfa_haptic_ref_gain(void *buf, int cmd_size)
{
	union afe_spkr_prot_config afe_spk_config;
	int32_t port_id = AFE_PORT_ID_QUATERNARY_MI2S_RX;
	...
	if (afe_spk_prot_prepare(port_id, 0,
			AFE_PARAM_ID_TFADSP_RX_SET_HAPTIC_GAIN,
			&afe_spk_config)) {
		pr_err("%s: AFE_TFADSP_RX_SET_BYPASS failed\n",
				   __func__);
	}
