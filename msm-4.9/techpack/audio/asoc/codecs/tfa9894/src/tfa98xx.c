...
static void tfa98xxCalibration(void);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 6 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);

	mutex_lock(&tfa98xx->dsp_lock);

	dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 7 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 1 tfa98xx->dsp_init : %d ,  tfa98xx->dsp_fw_state =  %d \n", __func__, tfa98xx->dsp_init,  tfa98xx->dsp_fw_state );

		if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK)
			return 0;

		dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 2 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);

		mutex_lock(&tfa98xx->dsp_lock);

		dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 3 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);

		tfa_dev_stop(tfa98xx->tfa);

	    dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 4 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);

		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;

		mutex_unlock(&tfa98xx->dsp_lock);

		dev_dbg(&tfa98xx->i2c->dev, "%s: nubia debug 5 tfa98xx->dsp_init : %d\n", __func__, tfa98xx->dsp_init);
