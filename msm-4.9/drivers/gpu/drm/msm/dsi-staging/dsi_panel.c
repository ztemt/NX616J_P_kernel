...
enum dsi_dsc_ratio_type {
 ...
 int switch_panel_res=0;
 unsigned int lcd_states= DISPLAY_PRIMARY;
 extern struct nubia_disp_type nubia_disp_val;
 int lcd_panel_index = 0;
 int front_panel_width = 0;
 int front_panel_hight = 0;
 int back_panel_width = 0;
 int back_panel_hight = 0;
 const char *cmd_set_prop_map[DSI_CMD_SET_MAX];
 bool nubia_back_lcd_exist = true;
#endif
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->switch_config.sub_lcd_fd_gpio)) {
		rc = gpio_request(panel->switch_config.sub_lcd_fd_gpio, "sub_lcd_fd_gpio");
		if (rc) {
				pr_err("request for sub_lcd_fd_gpio failed, rc=%d\n", rc);
			goto error_release_sub_lcd_fd_gpio;
			}
	}
...
error_release_sub_lcd_fd_gpio:
	if (gpio_is_valid(r_config->sub_lcd_fd_gpio))
		gpio_free(r_config->sub_lcd_fd_gpio);
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	for (i = 0; i < r_config->count; i++) {
		...
		if(switch_panel_res)
			...
			gpio_set_value(r_config->sub_lcd_reset_gpio,0);
...
static int nubia_dsi_panel_power_on(struct dsi_panel *panel)
{
    ...
    if((panel->cur_mode->timing.h_active == front_panel_width)&&(panel->cur_mode->timing.v_active == front_panel_hight)&&(switch_panel_res == 1)){
	     switch_panel_res = 0;
	     pr_err("SurfaceFligner crashed, reset lcd state to %d",switch_panel_res);
    }
	if(switch_panel_res){
	...
	if(switch_panel_res){
		rc = gpio_direction_output(panel->switch_config.sub_lcd_fd_gpio,1);
		gpio_set_value(panel->switch_config.sub_lcd_fd_gpio,0);
	}
	...
	if(switch_panel_res == 0){
		...
		if(!nubia_is_smwp_en){
			...
			if (rc) {
				pr_err("[%s] prim-panel power off failed to enable vregs, rc=%d\n", panel->name, rc);
				//goto error_disable_vregs;
			}
		}
	}
	...
	if(!switch_panel_res){
		...
		if(!nubia_is_smwp_en){
			if (gpio_is_valid(r_config->sub_tp_reset_gpio)){
				gpio_set_value(r_config->sub_tp_reset_gpio, 0);
			}
		}
	}

	if(!switch_panel_res && !nubia_is_smwp_en){
	...
	if(switch_panel_res==0){
		rc = gpio_direction_output(panel->switch_config.sub_lcd_fd_gpio,1);
		gpio_set_value(panel->switch_config.sub_lcd_fd_gpio,1);
		rc = dsi_pwr_enable_regulator(&panel->sub_panel_power_info, false);
		if (rc) {
			pr_err("[%s] sbu-panel power off failed to enable vregs, rc=%d\n", panel->name, rc);
			//goto exit;
		}
		usleep_range(2000,2000);
		rc = gpio_direction_output(panel->switch_config.sub_lcd_power_1p8_gpio,1);
	    if (rc)
			pr_err("unable to set dir for sub_lcd_power_1p8_gpio rc=%d\n", rc);

		gpio_set_value(panel->switch_config.sub_lcd_power_1p8_gpio,0);//gpio 35
	}
	if(switch_panel_res){
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->switch_config.lcd_switch_en_gpio))
		gpio_set_value(panel->switch_config.lcd_switch_en_gpio, 1);
	if (gpio_is_valid(panel->switch_config.lcd_switch_gpio))
		gpio_set_value(panel->switch_config.lcd_switch_gpio, 0);

	usleep_range(2000,2000);
	if (gpio_is_valid(panel->switch_config.sub_lcd_power_1p8_gpio))
	gpio_set_value(panel->switch_config.sub_lcd_power_1p8_gpio, 0);
#else
	if (gpio_is_valid(panel->reset_config.reset_gpio))
		gpio_set_value(panel->reset_config.reset_gpio, 0);
#endif
	...
	rc = dsi_pwr_enable_regulator(&panel->sub_panel_power_info, false);
	if(rc){
		pr_err(" error dsi_pwr_enable_regulator !, rc=%d\n", rc);
	} else {
		if(!nubia_is_smwp_en){
			rc = dsi_pwr_enable_regulator(&panel->power_info, false);
		}
	...
	u32 bl_lvl)
{
	...
	pr_err("dsi_panel_update_backlight++ bl_lvl= %d" , bl_lvl);
...
#ifdef CONFIG_NUBIA_SWITCH_LCD
	if((lcd_states == DISPLAY_POST_PRIMARY)||(lcd_states == DISPLAY_POST_SECOND)){
		pr_err("%s: it is switching do not need to setbacklight!\n", __func__);
		return rc;
	}

	pr_err("[test] backlight switch_panel_res = %d\n", switch_panel_res);
...
#ifdef CONFIG_NUBIA_SWITCH_LCD
      if(lcd_panel_index == 0){
	  	front_panel_width = mode->h_active;
		front_panel_hight= mode->v_active;
		pr_err("jbc front_panel_width:%d front_panel_hight:%d \n",mode->h_active, mode->v_active);
      	}
	  else if (lcd_panel_index == 1){
	  	back_panel_width = mode->h_active;
		back_panel_hight = mode->v_active;
		pr_err("jbc back_panel_width:%d back_panel_hight:%d\n",mode->h_active, mode->v_active);
	  	}
#endif  
...
#endif
#ifdef CONFIG_NUBIA_SWITCH_LCD
	"nuiba,mdss-dsi-sub-lcd-gamma-high",
	"nuiba,mdss-dsi-sub-lcd-gamma-low",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-normal",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-bright",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-outdoor",
	"nuiba,mdss-dsi-sub-lcd-command-aod-off",
	"nuiba,mdss-dsi-sub-lcd-command-aod-on",
#endif
};
...
#endif
#ifdef CONFIG_NUBIA_SWITCH_LCD
	"nuiba,mdss-dsi-sub-lcd-gamma-high-state",
	"nuiba,mdss-dsi-sub-lcd-gamma-low-state",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-normal-state",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-bright-state",
	"nuiba,mdss-dsi-prim-panel-command-rgbw-outdoor-state",
	"nuiba,mdss-dsi-sub-lcd-command-aod-off-state",
	"nuiba,mdss-dsi-sub-lcd-command-aod-on-state",
#endif
};
...
//Switch enable pin
	panel->switch_config.lcd_switch_en_gpio = of_get_named_gpio(of_node,
		"qcom,platform-lcd-switch-en-gpio", 0);
	if (!gpio_is_valid(panel->switch_config.lcd_switch_en_gpio)) {
...
//Switch pin
	panel->switch_config.lcd_switch_gpio = of_get_named_gpio(of_node,
		"qcom,platform-lcd-switch-gpio", 0);
	if (!gpio_is_valid(panel->switch_config.lcd_switch_gpio)) {
...
//Sub-LCD 1.8V power-supply
	panel->switch_config.sub_lcd_power_1p8_gpio = of_get_named_gpio(of_node,
		"qcom,platform-sub-lcd-power-1p8-gpio", 0);
	if (!gpio_is_valid(panel->switch_config.sub_lcd_power_1p8_gpio)) {
...
//Sub-lcd fd pin
	panel->switch_config.sub_lcd_fd_gpio = of_get_named_gpio(of_node,
		"qcom,platform-sub-lcd-fd-gpio", 0);
	if (!gpio_is_valid(panel->switch_config.sub_lcd_fd_gpio)) {
		pr_err("[%s] failed get sub_lcd_power_1p8_gpio gpio, rc=%d\n", panel->name, rc);
		//rc = -EINVAL;
		//goto error;
...
//tp reset
	panel->switch_config.sub_tp_reset_gpio = of_get_named_gpio(of_node,
		"qcom,platform-sub-tp-reset-gpio", 0);
	if (!gpio_is_valid(panel->switch_config.sub_tp_reset_gpio)) {
	...
	panel->reset_config.sub_lcd_reset_gpio= of_get_named_gpio(of_node,
		"qcom,platform-sub-lcd-reset-gpio", 0);
	if (!gpio_is_valid(panel->reset_config.sub_lcd_reset_gpio)) {
...
#ifdef CONFIG_NUBIA_SWITCH_LCD
int nubia_dsi_panel_rgbw(struct dsi_panel *panel, uint32_t rgbw_modes)
{
	int rc = 0;
	...
	switch(rgbw_modes){
		case RGBW_NORMAL:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_NORMAL);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_NORMAL cmds, rc=%d\n",
				       panel->name, rc);
			}
			break;
		case RGBW_BRIGHT:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_BRIGHT);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_BRIGHT cmds, rc=%d\n",
				       panel->name, rc);
			}
			break;
		case RGBW_OUTDOOR:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_OUTDOOR);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_OUTDOOR cmds, rc=%d\n",
				       panel->name, rc);
			}
			break;
		default:
...
int nubia_dsi_panel_aod(struct dsi_panel *panel, uint32_t state)
{
	...
	if(!switch_panel_res){
		...
		pr_err("Primary LCD can NOT support AOD mode !\n");
...
#ifdef CONFIG_NUBIA_AOD_MODE_GESTURE_EN
			dsi_panel_notifier(MSM_DRM_AOD_EVENT,MSM_DRM_AOD_OFF);
#endif
/*
			gpio_set_value(panel->switch_config.sub_lcd_fd_gpio,1);
...
#ifdef CONFIG_NUBIA_AOD_MODE_GESTURE_EN
			dsi_panel_notifier(MSM_DRM_AOD_EVENT,MSM_DRM_AOD_ON);
#endif
/*
			gpio_set_value(panel->switch_config.sub_lcd_fd_gpio,1);
			if (rc) {
...
int dsi_panel_switch(struct dsi_panel *panel)
{
    ...
    if(!nubia_back_lcd_exist){//here is unnormal state
	     switch_panel_res = 0;
		 lcd_states = DISPLAY_ONLY_PRIMARY;
	     pr_err("Back lcd and tp is not exist,force switch to %d",switch_panel_res);
    } else { //here is normal state
		if(switch_panel_res==1){
			switch_panel_res=0;
			if(lcd_states != DISPLAY_POST_PRIMARY){
	      	lcd_states = DISPLAY_POST_PRIMARY;
	      	pr_err("%s ---- Kernel set lcd_states =%d \n",__func__,lcd_states);}
		} else {
			switch_panel_res=1;
			if(lcd_states != DISPLAY_POST_SECOND){
			lcd_states = DISPLAY_POST_SECOND;
			pr_err("%s +++ Kernel set lcd_states =%d \n",__func__,lcd_states);}
		}
	}
	pr_err("%s Panel switching switch_panel_resource =%d lcd_states =%d \n",__func__,switch_panel_res,lcd_states);
	rc = nubia_dsi_panel_power_on(panel);
...
#ifdef CONFIG_NUBIA_SWITCH_LCD
	if(nubia_disp_val.rgbw_mode == RGBW_NORMAL) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_NORMAL);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_NORMAL cmds, rc=%d\n",
				       panel->name, rc);
		}
	} else if(nubia_disp_val.rgbw_mode == RGBW_BRIGHT) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_BRIGHT);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_BRIGHT cmds, rc=%d\n",
			       panel->name, rc);
		}
	} else if(nubia_disp_val.rgbw_mode == RGBW_OUTDOOR) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRIM_LCD_RGBW_MODE_OUTDOOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SET_PRIM_LCD_RGBW_MODE_OUTDOOR cmds, rc=%d\n",
			       panel->name, rc);
