...
void tfa9xxx_haptic_clock(struct tfa9xxx *drv, bool on)
{
	...
	if (on) {
		...
		drv->clk_users--;
		if (drv->clk_users == 0 && drv->bck_running == false)
...
#ifdef NUBIA_HAPTIC_TFA9XXX
	if (val > 0) {
		dev_info(&drv->i2c->dev, "%s: duration = %d, drv->object_index = %d \n", __func__, val, drv->object_index);
		if(val > 50){
			current_mode = drv->object_index;
			drv->object_index = 3;
