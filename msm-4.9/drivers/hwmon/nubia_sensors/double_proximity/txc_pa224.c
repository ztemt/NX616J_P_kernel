...
#define TXC_PROXNAME "proximity_back"
...
/* Promblem Number: PR000     Author:xuxiaohua,   Date:2018/6/25
...
static int pa224_read_file(char *filename, u8* param,int count)
{
	struct file  *fop;
	mm_segment_t old_fs;
	...
	fop->f_pos = 0;
/* Promblem Number: PR000     Author:xuxiaohua,   Date:2018/7/4
   Description    : fix strelen buffer overview bug; change sizeof or change
                    int count param */
   //   vfs_retval = vfs_read(fop, param, 2, &fop->f_pos);
...
/* Promblem Number: PR000     Author:xuxiaohua,   Date:2018/7/4
   Description    : fix strelen buffer overview bug; change sizeof or change
                    int count param */
	ret = pa224_read_file(filename, param,sizeof(param));
	if (ret < 0) {
...
static int pa224_run_calibration(struct i2c_client *client)
{
	...
	for (i = scale; i < ArySize - scale; i++)
	{
		SENSOR_LOG_INFO("sawp temp_pdata = [%d,%d]\n",i,temp_pdata[i]);
		sum_of_pdata = sum_of_pdata + temp_pdata[i];
	}
				...
				struct device_attribute *attr, char *buf)
{
    ...
    //virtual_proximity_report_event(data->ps_status, REL_RZ);
...
static ssize_t pa224_prox_offset_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
    struct pa224_data *data = dev_get_drvdata(dev);
    return sprintf(buf, "%d\n",data->crosstalk);
...
/* Device init */
static ssize_t pa224_show_dev_init(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
	return sprintf(buf, "dev init\n");
}
...
static struct device_attribute attrs_prox_device[] = {
	__ATTR(enable, 0664, pa224_show_enable_ps_sensor, pa224_store_enable_ps_sensor),
     __ATTR(prox_offset, 0440, pa224_prox_offset_show,NULL),
	__ATTR(delay, 0664, pa224_show_ps_poll_delay, pa224_store_ps_poll_delay),
	__ATTR(prox_value, 0440, pa224_show_ps, NULL),
	...
	__ATTR(prox_init, 0664, pa224_show_dev_init, pa224_store_dev_init),
...
static void pa224_work_func_irq(struct work_struct *work)
{
	...
	//wake_lock_timeout(&data->pa224_wake_lock, msecs_to_jiffies(1000));
	  __pm_wakeup_event(&data->pa224_wake_lock, msecs_to_jiffies(100));
...
static irqreturn_t pa224_irq(int irq, void *handle)
{
	...
	//wake_lock_timeout(&data->pa224_wake_lock, msecs_to_jiffies(1000));
	SENSOR_LOG_INFO("pa224_irq  yxc enter\n");
	__pm_wakeup_event(&data->pa224_wake_lock, msecs_to_jiffies(100));
	pa224_check_intr(client);
		...
		const struct i2c_device_id *id)
{
	...
	proximity_class = class_create(THIS_MODULE, TXC_PROXNAME);
	data->proximity_dev = device_create(proximity_class, NULL, pa224_proximity_dev_t, &pa224_driver ,TXC_PROXNAME);
    ...
    data->ps_input_dev = virtual_ps_input_dev;
	...
	*/
	...
	wakeup_source_init(&data->pa224_wake_lock,"pa224_wake_lock");
	/*Device Initialize*/
...
//	wake_lock_destroy(&data->pa224_wake_lock);
	wakeup_source_trash(&data->pa224_wake_lock);
