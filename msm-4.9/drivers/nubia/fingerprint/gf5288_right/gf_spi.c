...
#define GF_SPIDEV_NAME     "goodix,fingerprint_r"
/*device name after register in charater*/
#define GF_DEV_NAME            "goodix_fp_r"
#define	GF_INPUT_NAME	    "gf5288_r"	/*"goodix_fp" */

#define	CHRD_DRIVER_NAME	"goodix_fp_spi_r"
#define	CLASS_NAME		    "goodix_fp_r"
...
static struct gf_dev gf_right;

struct gf_key_map right_maps[] = {
	{ EV_KEY, GF_KEY_INPUT_HOME },
	{ EV_KEY, GF_KEY_INPUT_MENU },
	{ EV_KEY, GF_KEY_INPUT_BACK },
	{ EV_KEY, GF_KEY_INPUT_POWER },
	{ EV_KEY, GF_KEY_INPUT_F4 },
...
static int irq_setup(struct gf_dev *gf_dev)
{
	...
	gf_dev->irq = gf_right_irq_num(gf_dev);
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	if (GF_KEY_HOME == gf_key->key) {
		...
		key_input = GF_KEY_INPUT_F4;	// for super screenshot.
...
static int gf_open(struct inode *inode, struct file *filp)
{
    ...
    FP_LOG_INFO("Goodix fingerprint enter\n");
	...
	if (status == 0) {
		if (status == 0) {
			...
			FP_LOG_INFO("Succeed to open device. irq = %d\n",
					gf_dev->irq);
			if (gf_dev->users == 1) {
				status = gf_right_parse_dts(gf_dev);
				if (status)
					goto err_parse_dt;

				status = gf_pinctrl_right_init(gf_dev);
				status = gf_pinctrl_right_select(gf_dev, true);
		...
		unsigned long val, void *data)
{
	...
	FP_LOG_INFO("go to the goodix_fb_state_chg_callback brank = %d\n", *(int *)(evdata->data));
	if (val != FB_EARLY_EVENT_BLANK_FP)
		return 0;
	gf_dev = container_of(nb, struct gf_dev, notifier);
	if (evdata && evdata->data && val == FB_EARLY_EVENT_BLANK_FP && gf_dev) {
...
//extern int msm_drm_switch_register_client(struct notifier_block *nb);
...
#endif
{
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(right_maps); i++)
			input_set_capability(gf_dev->input, right_maps[i].type, right_maps[i].code);
    ...
    //msm_drm_switch_register_client(&gf_dev->notifier);
	msm_drm_panel_register_client(&gf_dev->notifier);
...
#if defined(USE_SPI_BUS)
static struct spi_driver gf_right_driver = {
#elif defined(USE_PLATFORM_BUS)
static struct platform_driver gf_right_driver = {
#endif
	.driver = {
		.name = GF_DEV_NAME,
		.owner = THIS_MODULE,
...
static int __init gf_right_init(void)
{
	int status;
	...
	if (IS_ERR(gf_class)) {
		unregister_chrdev(SPIDEV_MAJOR, gf_right_driver.driver.name);
		FP_LOG_ERROR("Failed to create class.\n");
		return PTR_ERR(gf_class);
	}
#if defined(USE_PLATFORM_BUS)
	status = platform_driver_register(&gf_right_driver);
#elif defined(USE_SPI_BUS)
	status = spi_register_driver(&gf_right_driver);
#endif
	if (status < 0) {
...
module_init(gf_right_init);

static void __exit gf_right_exit(void)
{
#ifdef GF_NETLINK_ENABLE
	netlink_right_exit();
#endif
#if defined(USE_PLATFORM_BUS)
	platform_driver_unregister(&gf_right_driver);
#elif defined(USE_SPI_BUS)
	spi_unregister_driver(&gf_right_river);
#endif
	class_destroy(gf_class);
	unregister_chrdev(SPIDEV_MAJOR, gf_right_driver.driver.name);
}
module_exit(gf_right_exit);
