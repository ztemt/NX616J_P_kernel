	...
	if (status == 0) {
		if (status == 0) {
			...
			if (gf_dev->users == 1) {
				...
				if (status) {
					FP_LOG_ERROR("gf_parse_dts status=%d\n",status);
					goto err_parse_dt;
				}
				...
				if (status) {
					FP_LOG_ERROR("irq_setup status=%d\n",status);
					goto err_irq;
				}
		...
		unsigned long val, void *data)
{
	...
	if (val != FB_EARLY_EVENT_BLANK_FP) {
		FP_LOG_INFO("go to the goodix_fb_state_chg_callback val != FB_EARLY_EVENT_BLANK_FP!!\n");
		return 0;
	}

	if (evdata && evdata->data && val == FB_EARLY_EVENT_BLANK_FP && gf_dev) {
		blank = *(int *)(evdata->data);
		FP_LOG_INFO("go to the goodix_fb_state_chg_callback value=%d blank=%d\n", (int)val, blank);
...
int gf_power_init(struct gf_dev *dev, bool on)
{
    ...
    if (on) {
		...
		if (IS_ERR(dev->vdd)) {
			rc = PTR_ERR(dev->vdd);
			FP_LOG_ERROR("Regulator get failed vdd rc=%d\n", rc);
			return rc;
		}
		...
		if (!IS_ERR_OR_NULL(dev->vdd)) {
			if (regulator_count_voltages(dev->vdd) > 0) {
				...
				if (rc) {
					FP_LOG_ERROR("Regulator set failed vdd rc=%d\n",rc);
...
static int autodetect_finger(struct gf_dev *gf_dev)
{
	int id_gpio = 0;
	int rc = -EINVAL;

	rc = gf_power_control(gf_dev, true);
    if (rc) {
            FP_LOG_ERROR("power true failed. rc =%d\n", rc);
            gf_dev->power_on = false;
	    return -EFAULT;
    } else {
            FP_LOG_ERROR("power true success \n");
    }

	msleep(5);
	
	gf_dev->id_gpio = of_get_named_gpio(gf_dev->spi->dev.of_node,"goodix,goodix_id",0);
	if(!gpio_is_valid(gf_dev->id_gpio)) {
		FP_LOG_ERROR("id GPIO is invalid.\n");
		return -1;
	}
	FP_LOG_INFO("gf::id_gpio:%d\n", gf_dev->id_gpio);
	rc = gpio_request(gf_dev->id_gpio, "goodix_id");
	if(rc) {
	...
	id_gpio = gpio_get_value(gf_dev->id_gpio);
	if (!id_gpio) {
		FP_LOG_INFO("gf::detect goodix finger success!!\n");
		...
		FP_LOG_INFO("gf::detect goodix finger failed!!\n");
		gpio_free(gf_dev->id_gpio);
...
#endif
{
	...
	if (status) {
		FP_LOG_ERROR("unable to configure regulator\n");
		goto error_power_init;
	}

	#ifdef FEATURE_NUBIA_FINGERPRINT_AUTODETECT
	if (autodetect_finger(gf_dev) != 0) {
