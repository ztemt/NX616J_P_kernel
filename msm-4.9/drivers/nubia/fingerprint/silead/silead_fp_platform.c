...
static int silfp_keyevent(struct silfp_data	*fp_dev, struct fp_dev_key_t *pkey)
{
    int ret = -EFAULT;
    //int i;
    static int silead_early = KEY_F3;
    int silead_key = KEY_F3;
	...
	switch( pkey->value ) {
		case NAV_KEY_CAMERA:
			silead_key = KEY_CAMERA;
			silead_early = KEY_CAMERA;
			break;
		case NAV_KEY_F3:
			silead_key = KEY_F3;
			silead_early = KEY_F3;
			break;
		case NAV_KEY_LONGPRESS:
			if(silead_early == KEY_CAMERA)
			{
				silead_key = KEY_CAMERA;
			}
			else if(silead_early == KEY_F3)
			{
				silead_key = KEY_F3;
			}
			silead_early = KEY_F3;
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] unregister pkey->value %d\n", __func__,pkey->value);
	if (NAV_KEY_FLAG_CLICK == pkey->flag) {
		if(pkey->value == NAV_KEY_CAMERA)
		{
			input_report_key(fp_dev->input, silead_key, NAV_KEY_FLAG_DOWN);
			input_sync(fp_dev->input);
			input_report_key(fp_dev->input, silead_key, NAV_KEY_FLAG_UP);
			input_sync(fp_dev->input);
		}
		else if(pkey->value == NAV_KEY_F3)
		{
			input_report_key(fp_dev->input, silead_key, NAV_KEY_FLAG_DOWN);
			input_sync(fp_dev->input);
		}
		else if(pkey->value == NAV_KEY_LONGPRESS)
		{
			if(silead_key == KEY_F3)
			{
				input_report_key(fp_dev->input, silead_key, NAV_KEY_FLAG_UP);
				input_sync(fp_dev->input);
			}
