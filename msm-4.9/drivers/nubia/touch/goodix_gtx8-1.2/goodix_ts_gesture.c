...
/**
*   show for internal gesture status
*/
int gsx_gesture_get_status(void){
...
static ssize_t gsx_gesture_enable_show(struct goodix_ext_module *module,
		char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&gsx_gesture->enable));
}

extern void goodix_resume_state_set(struct goodix_ts_core *core_data, int v);

static void gsx_gesture_enable_set(int v)
{
	int value = v ? 1:0;
	int en = atomic_read(&gsx_gesture->enable);
	int registered = atomic_read(&gsx_gesture->registered);
	int suspend = 0;
	struct goodix_ts_core *core_data = gsx_gesture->core_data;

	if(value == en){
		ts_info("Gesture module is %d now, not need set!\n", en);
		return;
	}

	if(registered == 0){
		ts_err("Gesture module not registered!\n");
		return;
	}

	suspend = atomic_read(&(core_data->suspended));


	if(suspend){
		if(suspend == 1){
			ts_info("need resume tp first!!\n");
			goodix_resume_state_set(core_data, 1);
		}

		do{
			ts_info("wait goodix resume finish!!\n");
			msleep(1);
		}while(atomic_read(&(core_data->suspended)));
	}

	ts_info("set gsx gesture value to %d\n", value);
	atomic_set(&gsx_gesture->enable, value);

	if(suspend == 1){
		ts_info("Gesture set ok, back the tp to suspend!!\n");
		goodix_resume_state_set(core_data, 0);
	}

	ts_info("Gesture module %s ok!", value ? "enable":"disable");
}
...
void second_touch_aod_mode_set(int v)
{
	struct goodix_ts_core *core_data = gsx_gesture->core_data;

	pr_info("%s:enter\n", __func__);
	if(v){
		gsx_gesture_enable_set(1);
		goodix_resume_state_set(core_data, 0);
	}else{
		goodix_resume_state_set(core_data, 1);
		do{
...
int goodix_unregister_gesture(void){
	int ret = 0;
	if (!atomic_read(&gsx_gesture->registered)) {
		ts_debug("Gesture module has aready unregistered");
		return -1;
		...
		struct goodix_ext_module *module)
{
	...
	gsx_gesture->core_data = core_data;
	...
	struct goodix_ext_module *module)
{
	...
	if(atomic_read(&gsx_gesture->enable) == 0){
		return EVT_CONTINUE;
	}
	...
	if (checksum != 0) {
		...
		ts_err("Gesture data %*ph", (int)sizeof(temp_data), temp_data);
		goto re_send_ges_cmd;
	}
...
static int __init goodix_gsx_gesture_init(void)
{
	...
	if (result == 0)
		atomic_set(&gsx_gesture->registered, 1);
	atomic_set(&gsx_gesture->enable, 0);
