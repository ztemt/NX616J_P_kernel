...
#define GOOIDX_INPUT_PHYS		"second_touch/input0"
#define PINCTRL_STATE_ACTIVE    "pmx_ts_active"
#define PINCTRL_STATE_SUSPEND   "pmx_ts_suspend"

extern int goodix_start_cfg_bin(struct goodix_ts_core *ts_core);
extern int second_touch_aod_mode_set(int val);
...
#ifdef GOODIX_TS_PALM
extern int g_palm_enabled;
/* show irq infomation */
static ssize_t goodix_ts_palm_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	int r;
...
/* enable/disable irq */
static ssize_t goodix_ts_palm_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf,
		size_t count)
{
...
#ifdef GOODIX_TS_DYNAMIC_RESUME_SUSPEND
static ssize_t goodix_ts_resume_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	struct goodix_ts_core *core_data =
		dev_get_drvdata(dev);
	int r;
	if (core_data->back_resume)
		r = snprintf(buf, PAGE_SIZE, "resume\n");
	else
		r = snprintf(buf, PAGE_SIZE, "suspended\n");
	if (r < 0)
		return -EINVAL;
...
static ssize_t goodix_ts_resume_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf,
		size_t count)
{
	...
	ts_err("resume: %d", en);
	if(en){
		core_data->back_resume = true;
		queue_work(core_data->resume_wq, &core_data->resume_work);
	}
	else{
		core_data->back_resume = false;
...
void goodix_resume_state_set(struct goodix_ts_core *core_data, int v)
{
	if(v){
		queue_work(core_data->resume_wq, &core_data->resume_work);
	}else{
...
static DEVICE_ATTR(read_cfg, S_IRUGO|S_IRGRP|S_IROTH, goodix_ts_read_cfg_show, NULL);
static DEVICE_ATTR(irq_info, S_IRUGO | S_IWUSR | S_IWGRP,
		goodix_ts_irq_info_show, goodix_ts_irq_info_store);
#ifdef GOODIX_TS_PALM
static DEVICE_ATTR(palm, S_IRUGO | S_IWUSR | S_IWGRP, goodix_ts_palm_show, goodix_ts_palm_store);
#endif
#ifdef GOODIX_TS_DYNAMIC_RESUME_SUSPEND
static DEVICE_ATTR(resume, S_IRUGO | S_IWUSR | S_IWGRP, goodix_ts_resume_show, goodix_ts_resume_store);
#endif
...
/* event notifier */
static BLOCKING_NOTIFIER_HEAD(ts_notifier_exist);
/**
 * goodix_ts_register_client - register a client notifier
 * @nb: notifier block to callback on events
 *  see enum ts_notify_event in goodix_ts_core.h
 */
int goodix_ts_exist_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ts_notifier_exist, nb);
}
EXPORT_SYMBOL(goodix_ts_exist_register_notifier);
 ...
 *	see enum ts_notify_event in goodix_ts_core.h
 */
int goodix_ts_exist_unregister_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&ts_notifier_exist, nb);
}
EXPORT_SYMBOL(goodix_ts_exist_unregister_notifier);

/**
 * fb_notifier_call_chain - notify clients of fb_events
 *	see enum ts_notify_event in goodix_ts_core.h
 */
int goodix_ts_exist_blocking_notify(unsigned long val)
{
	return blocking_notifier_call_chain(&ts_notifier_exist,
			val, NULL);
}
EXPORT_SYMBOL_GPL(goodix_ts_exist_blocking_notify);


#ifdef GOODIX_TS_PALM
extern int himax_touch_is_active(void);
/**
 * goodix_ts_palm_report - report palm event to input subsystem
 *
 * @dev: input device palm
 * return: 0 ok, <0 failed
 */
static int goodix_ts_palm_report(struct input_dev *dev)
{
	ts_info(" %s",__func__);
	if(himax_touch_is_active()){
		ts_info("the front touch is active, not need back touch palm function!!\n");
		return 0;
	}
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	list_for_each_entry(ext_module, &goodix_modules.head, list) {
		...
		if (r == EVT_CANCEL_IRQEVT){
			ts_info("%d, EVT_CANCEL_IRQEVT for ext_module: %s!!!!!!",(int)(core_data->irq_trig_cnt), ext_module->name);
			mutex_unlock(&goodix_modules.mutex);
			return IRQ_HANDLED;
		}
	}
...
#ifdef GOODIX_TS_PALM
		if (ts_event->event_type == EVENT_PALM) {
			/* report PALM */
			goodix_ts_palm_report(core_data->input_dev);
		}
#endif
	}
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	struct goodix_ts_device *ts_dev = core_data->ts_dev;
	int r;
#ifdef NUBIA_TOUCH_RESUME_WORKQUEUE
	flush_work(&core_data->resume_work);
#endif
	ts_info("Suspend start");

	if (atomic_read(&core_data->suspended) == 1){
		ts_info("already Suspended!!!");
		return -1;
	}
	/*
	...
	mutex_unlock(&goodix_modules.mutex);
	if(!gsx_gesture_get_status()){
		gpio_direction_output(core_data->ts_dev->board_data->reset_gpio, 0);
		ts_err("goodix_ts_power_off");
		r = goodix_ts_power_off(core_data);
		if (r < 0)
			goto out;
	}

out:
	input_report_key(core_data->input_dev, KEY_F13, 1);
	input_sync(core_data->input_dev);
	input_report_key(core_data->input_dev, KEY_F13, 0);
	input_sync(core_data->input_dev);
...
#ifndef NUBIA_TOUCH_RESUME_WORKQUEUE
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	if(!gsx_gesture_get_status())
	{
		ts_err("goodix_ts_power_on");
		r = goodix_ts_power_on(core_data);
		if (r < 0)
			goto out;
		usleep_range(12000, 12000);
		gpio_direction_output(core_data->ts_dev->board_data->reset_gpio, 1);
...
static void resume_work_func(struct work_struct *work)
{
	...
	ts_info("resume workqueue start\n");
#ifdef CONFIG_NUBIA_AOD_MODE_GESTURE_EN
	if (atomic_read(&core_data->suspended) == 0){
		ts_info("wfy: already resumed!!!");
		goto out;
	}else if(atomic_read(&core_data->suspended) == 2){
		ts_info("wfy: resume function not finish \n");
		goto out;
	}

	atomic_set(&core_data->suspended, 2);
#endif
...
#ifndef CONFIG_NUBIA_AOD_MODE_GESTURE_EN
	atomic_set(&core_data->suspended, 0);
#endif
	/* resume device */
	if (ts_dev && ts_dev->hw_ops->resume)
...
out:
	 ...
	 */
	goodix_ts_blocking_notify(NOTIFY_RESUME, NULL);
	ts_info("resume workqueue finish\n");
}

static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	queue_work(core_data->resume_wq,
                         ...
                         unsigned long event, void *data)
{
	...
	else if(event == MSM_DRM_AOD_EVENT){
            blank = fb_event->data;
            switch (*blank)
            {
                case MSM_DRM_AOD_ON:
                    core_data->aod_mode = 1;
                    break;
                case MSM_DRM_AOD_OFF:
                    core_data->aod_mode = 0;
                    break;
            }
               schedule_work(&core_data->aod_work);
        }
    }
...
static void aod_work_func(struct work_struct *work)
{
       struct goodix_ts_core *core_data  = container_of(work,
                       struct goodix_ts_core, aod_work);
       pr_err(" %s\n", __func__);
       second_touch_aod_mode_set(core_data->aod_mode);
}
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (!r)
		...
		goodix_ts_exist_blocking_notify(0x88);
		goto out;
	}
...
#ifdef NUBIA_TOUCH_RESUME_WORKQUEUE
	core_data->resume_wq =
			create_singlethread_workqueue("goodix_resume_workqueue");
	INIT_WORK(&core_data->resume_work, resume_work_func);
#endif
	INIT_WORK(&core_data->aod_work,aod_work_func);
	/*create sysfs files*/
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	flush_workqueue(core_data->resume_wq);
	destroy_workqueue(core_data->resume_wq);
#endif
