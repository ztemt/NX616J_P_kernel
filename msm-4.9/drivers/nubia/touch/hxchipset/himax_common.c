...
extern int hx83112b_jitter_switch(bool on);
extern int game_mode;
static atomic_t ts_irq_thread_running = ATOMIC_INIT(0);
static int g_himax_touch_active = 1;
...
{
#include "NX616J_NUBIA_BOE_FW_NEW.i"
};
#endif
#endif
...
#ifdef HX_FIX_NEW_PANEL
int g_i_new_FW_VER = 0;
int g_i_new_CFG_VER = 0;
int g_i_new_CID_MAJ = 0; //GUEST ID
int g_i_new_CID_MIN = 0; //VER for GUEST
#endif
#endif
...
int himax_touch_is_active(void)
{
	return g_himax_touch_active;
}

void ts_irq_thread_start(void)
{
	atomic_inc(&ts_irq_thread_running);
}

void ts_irq_thread_end(void)
{
	atomic_dec(&ts_irq_thread_running);
}
...
#ifdef HX_FIX_NEW_PANEL
    unsigned char* ImageBuffer_new = i_CTPM_FW_NEW;
    int fullFileLength_new = sizeof(i_CTPM_FW_NEW);
#endif
...
update_retry:
    ...
    {
#ifdef HX_FIX_NEW_PANEL
        if(ic_data->vendor_touch_cfg_ver < 0x30 && ic_data->vendor_touch_cfg_ver != 0){
            ret = fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client,ImageBuffer,fullFileLength,false);
        }else{
            ret = fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client,ImageBuffer_new,fullFileLength_new,false);
        }
#else
        ret = fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client,ImageBuffer,fullFileLength,false);
#endif
...
static void himax_report_points(struct himax_ts_data *ts)
{
    ...
    else
    {
        ...
        for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++){
        //   E("%s: nFinger_support:%d ,loop_i=%d leave\n", __func__,ts->nFinger_support, loop_i);
           input_mt_slot(ts->input_dev, loop_i);
           input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 0);
        }
...
{
	int loop_i = 0;
	input_report_key(ts->input_dev, KEY_F13, 1);
	input_sync(ts->input_dev);
	input_report_key(ts->input_dev, KEY_F13, 0);
	input_sync(ts->input_dev);
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++)
	{
#ifndef	HX_PROTOCOL_A
...
void himax_ts_work(struct himax_ts_data *ts)
{
    ...
    {
        __pm_wakeup_event(&ts->ts_SMWP_wake_sc, TS_WAKE_LOCK_TIMEOUT);
...
static void himax_fb_switch_register(struct work_struct *work){
    ...
    ret = msm_drm_panel_register_client(&ts->fb_notif);
    if (ret < 0) {
...
FW_force_upgrade:
#ifdef HX_FIX_NEW_PANEL
    I("vendor_touch_cfg_ver = %x", ic_data->vendor_touch_cfg_ver);
    if(ic_data->vendor_touch_cfg_ver < 0x30 && ic_data->vendor_touch_cfg_ver != 0){
        auto_update_flag |= (( ic_data->vendor_fw_ver < g_i_FW_VER ) || ( ic_data->vendor_config_ver < g_i_CFG_VER ));
    }else{
        auto_update_flag |= (( ic_data->vendor_fw_ver < g_i_new_FW_VER ) || ( ic_data->vendor_config_ver < g_i_new_CFG_VER ));
    }
#else
    auto_update_flag |= (( ic_data->vendor_fw_ver < g_i_FW_VER ) || ( ic_data->vendor_config_ver < g_i_CFG_VER ));
#endif
    if(ic_data->vendor_touch_cfg_ver == 0xB0){
        E("ic_data->vendor_touch_cfg_ver = %d", ic_data->vendor_touch_cfg_ver);
        E("g_i_FW_VER = %x  g_i_CFG_VER = %x", g_i_FW_VER, g_i_CFG_VER);
        auto_update_flag =0;
    }
...
#ifdef HX_SMART_WAKEUP
    ts->SMWP_enable=0;
    wakeup_source_init(&ts->ts_SMWP_wake_sc, HIMAX_common_NAME);
#endif
#ifdef HX_HIGH_SENSE
    ts->HSEN_enable=0;
#endif
...
#endif
#ifdef HX_SMART_WAKEUP
    wakeup_source_trash(&ts->ts_SMWP_wake_sc);
#endif
#ifdef  HX_CHIP_STATUS_MONITOR
    g_chip_monitor_data->HX_CHIP_MONITOR_EN = 0;
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
   ...
   // himax_resend_cmd_func(ts->suspended);
    ...
    irq_state = atomic_read(&ts_irq_thread_running);
    while(irq_state > 0){
        E("%s: himax irq thread not finish(%d),wait 1 ms\n", __func__, irq_state);
	msleep(1);
        irq_state = atomic_read(&ts_irq_thread_running);
    }
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
    ...
    himax_int_enable(ts->client->irq,1);
    //morgen add for game mode start
    I("game_mode=%d\n", game_mode);
    if (game_mode == 2)
        hx83112b_jitter_switch(1);
    //morgen add for game mode end
